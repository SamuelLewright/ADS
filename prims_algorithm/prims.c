	...
	while(traverse > 0){
		...
		for(i=0; i<nodes; i++){
			if( table[minDist][i] >0){
				if( table[minDist][i] < distance[i]){
					distance[i] = //distance[minDist] + 
					table[minDist][i];
					parent[i] = minDist + 1;
				}
			}
...
int main(){
    ...
    //reset all values before performind dijkstra
    for(i=0; i<nodes; i++){
	    distance[i] = 1000;
	    beenVisited[i] = 0;
	    cost[i] = 1;
	...
	scanf("%d", &S);
	distance[S - 1] = 0;
	//perform prim's with source S and number of nodes entered
	prim(S, nodes);
	return 0;
}
